...
class RaceEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        if state['checkpoint'] != self.prev_state['checkpoint']:
            reward += 50

        if state['progress'] == self.prev_state['progress']:
            self.not_moving += 1
            if self.not_moving >= 8:
                reward -= 1
            if self.not_moving >= 100:
                done = True
        else:
            self.not_moving = 0
            if state['progress'] > self.prev_state['progress']:
                reward += 2000 * (state['progress'] - self.prev_state['progress'])
                if state['progress'] < 0.4 and state['progress'] > 0.36:
                    reward += 5
