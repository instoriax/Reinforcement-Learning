...
public:
	...
	virtual float estimate(const board& b) const {
		// TODO
		float value=0;
		for(const auto& patt:isomorphic){
			size_t idx = indexof(patt, b);
			value += weight[idx];
		}
		return value;
	}
...
public:
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		for (path.pop_back(); path.size(); path.pop_back()) {
			state &mv = path.back();
			float error = target - estimate(mv.before_state());
			target = mv.reward() + update(mv.before_state(), alpha * error);
		}
	}
