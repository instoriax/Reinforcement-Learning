	...
	def update(self):
		...
		for _ in range(self.update_count):
			for start in range(0, sample_count, self.batch_size):
				...
				clip_ratio = torch.clamp(ratio, min=1-self.clip_epsilon, max=1+self.clip_epsilon)
				surrogate_loss = -torch.min(ratio * adv_train_batch, clip_ratio * adv_train_batch).mean()
